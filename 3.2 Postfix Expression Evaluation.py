def evaluate_postfix(expression):
    ...
    for token in tokens:
        if token in operators:
            ...
            # It's an operand, convert to float and push
            try:
                stack.append(float(token))
            except ValueError:
                raise ValueError(f"Invalid token in expression: {token}")
    
    # Final check: should only be one result in stack
    if len(stack) == 1:
        return stack[0]
    else:
...
if __name__ == "__main__":
    ...
    print(evaluate_postfix("3 4 +"))           # Output: 7.0
    print(evaluate_postfix("5 1 2 + 4 * + 3 -")) # Output: 14.0
    print(evaluate_postfix("7 2 / 3 *"))       # Output: 10.5
